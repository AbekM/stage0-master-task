...
public class AnimalTest {
  ...
  public void classAnimalExists () {
    Assertions.assertDoesNotThrow(() -> Class.forName("com.epam.OOP.Animal"),
        "Class Animal does not exist");
  }
  ...
  public void classAnimalHasAllNecessaryFields () {
    ...
    Field color = Arrays.stream(fields).filter(field -> field.getName().equals("color")).findFirst()
        .orElse(null);
    ...
    Field numberOfPaws = Arrays.stream(fields)
        .filter(field -> field.getName().equals("numberOfPaws")).findFirst().orElse(null);
    Assertions.assertNotNull(numberOfPaws, "Field numberOfPaws is missed");
    Assertions.assertTrue(Modifier.isPrivate(numberOfPaws.getModifiers()),
        "Field numberOfPaws is not private");
    Assertions.assertEquals(int.class, numberOfPaws.getType(),
        "Field numberOfPaws is not int type");

    Field hasFur = Arrays.stream(fields).filter(field -> field.getName().equals("hasFur"))
  ...
  public void classAnimalHasConstructorWithAllParameters () {
    try {
      ...
      Parameter color = Arrays.stream(parameters)
          .filter(parameter -> parameter.getType().equals(String.class)).findFirst().orElse(null);
      Assertions.assertNotNull(color, "Parameter for color is missed");

      Parameter numberOfPaws = Arrays.stream(parameters)
          .filter(parameter -> parameter.getType().equals(int.class)).findFirst().orElse(null);
      Assertions.assertNotNull(numberOfPaws, "Parameter for numberOfPaws is missed");

      Parameter hasFur = Arrays.stream(parameters)
          .filter(parameter -> parameter.getType().equals(boolean.class)).findFirst().orElse(null);
      ...
      Assertions.assertEquals("someColor", colorField.get(animal),
          "Not correct set up for color field");
      Assertions.assertEquals(123123, (int) numberOfPawsField.get(animal),
          "Not correct set up for numberOfPawsField field");
      Assertions.assertTrue((boolean) hasFurField.get(animal),
          "Not correct set up for hasFurField field");
  ...
  public void getDescriptionReturnCorrectString () {
    try {
      ...
      Assertions.assertEquals("This animal is mostly red. It has 3 paws and a fur.",
          getDescription.invoke(animal), "Method getDescription() does not work correctly");

      animal = (Animal) constructor.newInstance("blue", 1213, false);
      Assertions.assertEquals("This animal is mostly blue. It has 1213 paws and no fur.",
          getDescription.invoke(animal), "Method getDescription() does not work correctly");
  ...
  public void getDescriptionChangeResultDependingOnNumberOfPaws () {
    try {
      ...
      Assertions.assertEquals("This animal is mostly red. It has 1 paw and a fur.",
          getDescription.invoke(animal), "Method getDescription() does not work correctly");
    } catch (NoSuchMethodException | InstantiationException | IllegalAccessException |
             InvocationTargetException e) {
      Assertions.fail("Some Animal class configuration problems");
